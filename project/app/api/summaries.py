...
async def create_summary(payload: SummaryPayloadSchema) -> SummaryResponseSchema:
    ...
    with logfire.span("Create Summary"):
        ...
        logfire.info(f"Summary created successfully with id: {summary_id}")
...
async def read_summary(id: int = Path(..., gt=0)) -> SummarySchema:  # type: ignore
    with logfire.span("Read Summary"):
        logfire.info("Reading summary...")
...
async def read_all_summaries() -> list[SummarySchema]:  # type: ignore
    logfire.info("Reading all Summaries")
    return await crud.get_all()


@router.delete("/{id}/", response_model=SummaryResponseSchema)
async def remove_summary(id: int = Path(..., gt=0)) -> SummaryResponseSchema:
    with logfire.span(f"Delete Summary {id}"):
        logfire.info("Deleting summary...")
        summary = await crud.get(id)
        if not summary:
            logfire.error("Summary not found")
            raise HTTPException(status_code=404, detail="Summary not found")
        await crud.delete(id)
        logfire.info("Summary deleted successfully")
        return summary
